...
const UserBlogSubmission = require('./models/userBlogSubmissionSchema');
...
app.post('/api/image-posts/:id/comments', 
  extractDeviceId,
  [
    body('name').trim().notEmpty().withMessage('Name is required')
...
app.get('/api/image-posts/comments/:commentId/replies', async (req, res) => {
  try {
    const { commentId } = req.params;
    const { page = 1, limit = 10 } = req.query;
...
app.post('/api/image-posts/comments/:commentId/like',
  extractDeviceId,
  [
...
app.post('/api/image-posts/comments/:commentId/dislike',
  extractDeviceId,
  [
    body('email').trim().isEmail().withMessage('Valid email is required').normalizeEmail()
  ],
...
app.get('/api/image-posts/comments/:commentId/user-reaction',
  extractDeviceId,
  async (req, res) => {
    try {
      const { commentId } = req.params;
...
app.delete('/api/image-posts/comments/:commentId', 
  extractDeviceId,
  [
    body('email').trim().isEmail().withMessage('Valid email is required').normalizeEmail()
  ],
...
app.post('/api/image-posts/:id/author-comment',
  authenticateToken,
  [
    body('content').trim().notEmpty().withMessage('Comment content is required')
      .isLength({ max: 1000 }).withMessage('Comment cannot exceed 1000 characters'),
...
app.get('/api/admin/image-posts/:id/comments', 
  authenticateToken,
  async (req, res) => {
    try {
      const { id } = req.params;
...
app.put('/api/admin/image-comments/:commentId',
  authenticateToken,
  [
...
app.patch('/api/admin/image-comments/:commentId', 
  authenticateToken,
  [
...
app.delete('/api/admin/image-comments/:commentId',
  authenticateToken,
  async (req, res) => {
    try {
      const { commentId } = req.params;
...
app.get('/api/admin/image-posts/:id/comments/stats',
  authenticateToken,
  async (req, res) => {
    try {
      const { id } = req.params;
...
app.get('/api/admin/image-comments',
  authenticateToken,
  async (req, res) => {
    try {
...
app.post('/api/admin/image-comments/bulk-delete',
  authenticateToken,
  [
...
app.patch('/api/admin/image-comments/bulk-status',
  authenticateToken,
  [
    body('commentIds').isArray({ min: 1 }).withMessage('commentIds must be a non-empty array'),
    body('commentIds.*').isMongoId().withMessage('Invalid comment ID format'),
...
// Create new admin (Super Admin only) - WITH IMAGE UPLOAD
app.post('/api/admins', authenticateToken, isSuperAdmin, upload.single('profileImage'), async (req, res) => {
  try {
    const {
      name,
      email,
...
app.get('/api/admins/:id', authenticateToken, isSuperAdmin, async (req, res) => {
  try {
...
app.get('/api/admins/:id/image', async (req, res) => {
  try {
...
app.put('/api/admins/:id', authenticateToken, isSuperAdmin, upload.single('profileImage'), async (req, res) => {
  try {
    const { id } = req.params;
    const {
      name,
...
app.put('/api/profile/image', authenticateToken, upload.single('profileImage'), async (req, res) => {
  try {
    if (!req.file) {
...
app.delete('/api/profile/image', authenticateToken, async (req, res) => {
  try {
    const admin = await Admin.findById(req.user.admin_id);
...
// Update expertise
app.put('/api/profile/expertise', authenticateToken, async (req, res) => {
  try {
...
app.get('/api/admins/:id/public', async (req, res) => {
  try {
...
app.get('/api/admins/public/all', async (req, res) => {
  try {
...
app.get('/api/blog-submissions/status/:blogSubmissionId', async (req, res) => {
  try {
...
app.get('/api/blog-submissions/details/:blogSubmissionId', async (req, res) => {
  try {
    const { blogSubmissionId } = req.params;
...
app.get('/api/admin/blog-submissions', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.get('/api/admin/blog-submissions/:id', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.post('/api/admin/blog-submissions/:id/approve', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.post('/api/admin/blog-submissions/:id/reject', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.post('/api/admin/blog-submissions/:id/suggest-changes', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.delete('/api/admin/blog-submissions/:id', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.get('/api/admin/blog-submissions/stats/dashboard', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.post('/api/admin/blog-submissions/bulk/approve', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
...
app.post('/api/admin/blog-submissions/bulk/reject', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Access denied. Admin only.' });
