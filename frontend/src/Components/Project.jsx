...
const AdminProjectRequests = () => {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:5000/api/admin/project/requests', {
        headers: {
  ...
  const fetchRequestDetails = async (id) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:5000/api/admin/project/requests/${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const fetchFilePreview = async (requestId, fileIndex, mimetype) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/admin/project/requests/${requestId}/files/${fileIndex}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          },
  ...
  const handleAcknowledge = async (id) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/api/admin/project/requests/${id}/acknowledge`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleDeleteRequest = async (id) => {
    ...
    setActionLoading(true);
    try {
      await axios.delete(`http://localhost:5000/api/admin/project/requests/${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleDeleteAllRequests = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await axios.delete('http://localhost:5000/api/admin/project/requests', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
      });
